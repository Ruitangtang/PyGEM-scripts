...
# It's a copy of the run_calibration_AMIS_MB_FA_New.py, but with unique choice of the posterior distribution
...
#                output_df_all.loc[nglac, 'thick_measured_yn'] = fa_glac_data_reg.loc[fa_idx, 'thick_measured_yn']
            ...
            for nglac in np.arange(main_glac_rgi.shape[0]):
                ...
                if (not doadapt) or (not notlast): # TODO add the information of the txt infor about good and maximum iterations, with the Neff
                    ...
                    output_folder_post_params_unique = os.path.join(output_fp, 'parameter','Poster','Unique')
                    ...
                    if store_monthly_step:
                        ...
                        #massbalclim_TMS_model_array_monthly_post_weighted = np.average(np.asarray(massbalclim_TMS_model_array_monthly), axis=0,weights = Weights_unique)
                        #massbaltotal_TMS_model_array_monthly_post_weighted = np.average(np.asarray(massbaltotal_TMS_model_array_monthly), axis=0,weights = Weights_unique)
