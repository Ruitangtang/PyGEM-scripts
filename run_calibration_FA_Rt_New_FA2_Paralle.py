...
Copying from run_calibration_FA_Rt_New.py, with changes in the tau assemble, which was read from the result of run_calibration_FA_Rt_New.py
for the extra FA calibration after MCMC, as well as add the multiprocessing in Visualize_parameter_paralle 

Changes:
1. Read the tau_assemble from the result of run_calibration_FA_Rt_New.py
2. Read the modelprms from the MCMC calibration (kp,tbias,ddfsnow,ddfice,tsnow_threshold,precgrad)
3. Change the model setup to use the new tau_assemble
4. Add multiprocessing: Visualize_parameter_paralle and processing_tau
...
save_path_figure = pygem_prms.output_filepath + '/figures/second_cali_FA/'
...
def Visualize_parameter (model_function = None, k_bndhigh = None, k_bndlow = None, k_step = None,
                         k_name =None,calibrate_length = False,**kwags):
    ...
    # read from the first FA calibration
