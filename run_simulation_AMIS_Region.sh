...
do
    ...
    LOGFILE="/cluster/projects/nn11115k/Ruitang/SermeQ_OGGM_PyGEM/Output/Simulation/simulation_task${task_id}_log.txt"
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') Running task $task_id..." | tee -a "$LOGFILE"
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') Completed task $task_id." | tee -a "$LOGFILE"
    ...
    echo "$(date '+%Y-%m-%d %H:%M:%S') Total time for task $task_id: $DIFF_TIME_TASK seconds" | tee -a "$LOGFILE"
