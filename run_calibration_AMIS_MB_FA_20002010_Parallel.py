...
# It's a copy of the run_calibration_AMIS_MB_FA_20002010.py, but it is revised to be for  a list of glaciers,instead of a single glacier.
...
#                plt.show()
            ...
            try:
                ...
                save_path_figure_glac = os.path.join(save_path_figure, glacier_str.split('.')[0].zfill(2) + '/')
                if not os.path.exists(save_path_figure_glac):
    ...
    #                        print('calving_m3_since_y0:', ev_model.calving_m3_since_y0)
                        ...
                        save_path_figure_calving = os.path.join(save_path_figure_glac, f"{str(calving_k)}_{index_pariticles:.0f}")
...
def processing_parameters(model_function,kwargs,modelprms_MB_FA):
    ...
    output_df, reg_calving_gta_mod_good,_ , mb_years,mb_obs_mwea,mb_obs_mwea_err= model_function(modelprms_MB_FA = modelprms_MB_FA, do_DA_calib_Paralle = True, **kwargs)
...
def Visualize_parameter_paralle (model_function = None, parameters_dict = None,calibrate_timeseries = False,
                                 rgiid_ind = None, glacier_str = None,Visual_index = False, Debug_index =False, N_iteration = None,
    ...
    if calibrate_timeseries:
        ...
        # Use Pool with partial for additional arguments
            ...
            keys = [
                ...
                'frontal_ablation_mwea', 'frontal_ablation_mwea_timeseries','mb_years', 'mb_obs_mwea','mb_obs_mwea_err'
    ...
    if calibrate_timeseries:
        ...
        output_folder_params = os.path.join(save_path_parameter,  glacier_str.split('.')[0].zfill(2))
...
def Model_MB_FA_RT(model_function = reg_calving_flux,parameters_dict= None,rgiid_ind = None,
                   main_glac_rgi = None,glacier_str = None, fa_glac_data_reg= None,ignore_nan=False,
    ...
    output_prior =  Visualize_parameter_paralle (model_function = model_function,parameters_dict= parameters_dict,rgiid_ind = rgiid_ind,
                                                                    main_glac_rgi = main_glac_rgi, glacier_str =glacier_str,fa_glac_data_reg=fa_glac_data_reg,ignore_nan=ignore_nan,
    ...
    if store_result:
        ...
        if store_monthly_step:
            ...
            output_folder_monthly = os.path.join(save_path_modeloutput,'Monthly',glacier_str.split('.')[0].zfill(2))  # Assuming `pygem_prms.output_fp` exists
        ...
        output_folder_annual = os.path.join(save_path_modeloutput, 'Annual',glacier_str.split('.')[0].zfill(2))  # Assuming `pygem_prms.output_fp` exists
    ...
    # here is the full returen of the model, #TODO add the monthly results, or depends on the user's choice
        return (
            ...
            massbalclim_TMS_model_array_annual,massbaltotal_TMS_model_array_annual,FA_mwea_TMS_model_array_annual,
            velocity_at_calvingfront_model_array_annual,thickness_at_calvingfront_model_array_annual,
...
#                output_df_all.loc[nglac, 'thick_measured_yn'] = fa_glac_data_reg.loc[fa_idx, 'thick_measured_yn']
            ...
            for nglac in np.arange(main_glac_rgi.shape[0]):
                ...
                # set the output path for the single glacier
                save_path_figure_glac = os.path.join(save_path_figure, glacier_str.split('.')[0].zfill(2) + '/')
                if not os.path.exists(save_path_figure_glac):
                    os.makedirs(save_path_figure_glac,exist_ok=True)
                ...
                for j in range(max_iterations): #TODO DO THE APATED CALIBRATION HERE
                    ...
                    lengthchange_dLdt_model_array_annual, calving_flux_Gta_TMS_model_array_annual,calving_flux_Gta_average_model_array,massbalclim_TMS_model_array_annual,massbalclim_model_array,mb_obs_mwea,mb_obs_mwea_err = Model_MB_FA_RT(model_function = reg_calving_flux,parameters_dict= parameters_dict,
                                                                                         rgiid_ind = rgiid_ind,main_glac_rgi = main_glac_rgi_ind,glacier_str =glacier_str, fa_glac_data_reg= fa_glac_data_ind,
                    ...
                    output_folder_AMIS = os.path.join(save_path_AMISINFO,glacier_str.split('.')[0].zfill(2)) # Assuming `pygem_prms.output_fp` exists
                ...
                if (not doadapt) or (not notlast): # TODO add the information of the txt infor about good and maximum iterations, with the Neff
                    ...
                    output_folder_params_poster = os.path.join(save_path_parameter, 'Poster')
                    # Ensure directories exist
                    ...
                    output_folder_post_params_statis = os.path.join(save_path_parameter)  # Assuming `pygem_prms.output_fp` exists
                    # Check if directory exists, otherwise create it
                    ...
                    output_folder_post_params_unique = os.path.join(save_path_parameter,'Poster','Unique')
                    ...
                    output_folder_weights = os.path.join(save_path_modeloutput,'Poster', 'Weighted')  # Assuming `pygem_prms.output_fp` exists
                    # Check if directory exists, otherwise create it
                    ...
                    if store_monthly_step:
                        ...
                        output_folder_monthly = os.path.join(save_path_modeloutput , 'Monthly')  # Assuming `pygem_prms.output_fp` exists
                        ...
                        # --- save the monthly information in a hdf5 file and visulize the monthly information
                        output_folder_monthly_weighted = os.path.join(save_path_modeloutput, 'Poster','Weighted','Monthly')
