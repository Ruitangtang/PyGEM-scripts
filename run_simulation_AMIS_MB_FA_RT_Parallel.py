...
def simu_MB_FA_single_glac(n_iter = None, sim_iters = None,gdir =None, modelprms_all = None, tau_values =None,debug = False, glacier_str = None,
                           gdir_ref = None, glacier_rgi_table = None, nyears =None, nyears_ref = None, fs = None, glen_a_multiplier = None,
                           fls = None,reg_str = None, gcm_name= None,scenario = None,args = None):
    """
...
#                            mb_em_sims.append(mb_em_mwea)
    ...
    # return variables
    return (n_iter,successful_run, ev_model, diag, mbmod, surface_h_initial,length_change_m_annual,calving_m3_annual,nfls)


def simu_MB_FA(list_packed_vars,num_cores = 1):
    """
        ...
        # for batman in [0]:
            ...
            if (fls is not None) and (glacier_area_km2.sum() > 0):
                ...
                if num_iterations > 1:
                    # Parallel processing
                    if num_cores > 1:
                        subtasks = [(i,num_iterations) for i in range(num_iterations)]
                        with multiprocessing.Pool(num_cores) as p:
                            output_parallel = p.starmap(simu_MB_FA_single_glac, subtasks)
                    # Sequential processing
                    else:
                        for n_iter in range(num_iterations):
                            output_parallel = simu_MB_FA_single_glac(n_iter, num_iterations)
                ...
                if count_exceed_boundary_errors < pygem_prms.sim_iters:
                    ...
                    if pygem_prms.export_essential_data:
                        try:
                            ...
                            # Output statistics
                            output_glac_runoff_monthly_stats = calc_stats_array_Restruct(output_glac_runoff_monthly)
                            output_glac_area_annual_stats = calc_stats_array_Restruct(output_glac_area_annual)
                            output_glac_length_annual_stats = calc_stats_array_Restruct(output_glac_length_annual)
                            output_glac_length_change_annual_stats = calc_stats_array_Restruct(output_glac_length_change_annual)
                            output_glac_frontalablation_annual_stats = calc_stats_array_Restruct(output_glac_frontalablation_annual)
                            output_glac_mass_annual_stats = calc_stats_array_Restruct(output_glac_mass_annual)
                            output_glac_mass_bsl_annual_stats = calc_stats_array_Restruct(output_glac_mass_bsl_annual)
                            output_glac_ELA_annual_stats = calc_stats_array_Restruct(output_glac_ELA_annual)
                            output_offglac_runoff_monthly_stats = calc_stats_array_Restruct(output_offglac_runoff_monthly)
                            if pygem_prms.export_extra_vars:
                                output_glac_temp_monthly_stats = calc_stats_array_Restruct(output_glac_temp_monthly)
                                output_glac_prec_monthly_stats = calc_stats_array_Restruct(output_glac_prec_monthly)
                                output_glac_acc_monthly_stats = calc_stats_array_Restruct(output_glac_acc_monthly)
                                output_glac_refreeze_monthly_stats = calc_stats_array_Restruct(output_glac_refreeze_monthly)
                                output_glac_melt_monthly_stats = calc_stats_array_Restruct(output_glac_melt_monthly)
                                output_glac_frontalablation_monthly_stats = calc_stats_array_Restruct(output_glac_frontalablation_monthly)
                                output_glac_length_monthly_stats = calc_stats_array_Restruct(output_glac_length_monthly)
                                
                                output_glac_length_change_monthly_stats = calc_stats_array_Restruct(output_glac_length_change_monthly)
                                output_glac_massbaltotal_monthly_stats = calc_stats_array_Restruct(output_glac_massbaltotal_monthly)
                                output_glac_snowline_monthly_stats = calc_stats_array_Restruct(output_glac_snowline_monthly)
                                output_glac_mass_change_ignored_annual_stats = calc_stats_array_Restruct(output_glac_mass_change_ignored_annual)
                                output_offglac_prec_monthly_stats = calc_stats_array_Restruct(output_offglac_prec_monthly)
                                output_offglac_melt_monthly_stats = calc_stats_array_Restruct(output_offglac_melt_monthly)
                                output_offglac_refreeze_monthly_stats = calc_stats_array_Restruct(output_offglac_refreeze_monthly)
                                output_offglac_snowpack_monthly_stats = calc_stats_array_Restruct(output_offglac_snowpack_monthly)
