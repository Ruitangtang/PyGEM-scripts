...
It's a copy of run_calibraiton_FA_Rt_New.py, but for the calibtation will be based on PBS using both annual timeseries dLdt and FA.
...
# claibration dataset choice, if both are True, using the both dataset to calibrate the calving parameter, tau
calibrate_FA = True # calibrate using annual timeseries of frontal ablation
calibrate_dLdt = True # calibrate using annual timeseries of dLdt
...
def Visualize_parameter (model_function = None, k_bndhigh = None, k_bndlow = None, k_step = None, k_name =None,
    ...
    # # # # # # # # # # # # # # # # if calibrate_length:
    # # # # # # # # # # # # # # # #     fig = plt.figure(figsize=(10, 6))
    # # # # # # # # # # # # # # # #     # Normalize k_values to a colormap
    ...
    # # # # # # # # # # # # # # # #     # Plot each lengthchange_rate list with a different color based on k_values
    # # # # # # # # # # # # # # # #     for i, (k, rate) in enumerate(zip(k_values, lengthchange_rate)):
    # # # # # # # # # # # # # # # #         plt.plot(rate, label=f"k={k:.2f}", color=cmap(norm(k)))  # Plot with corresponding color
    # # # # # # # # # # # # # # # #     # Add colorbar to indicate k_values scale
    # # # # # # # # # # # # # # # #     sm = plt.cm.ScalarMappable(cmap=cmap, norm=norm)
    # # # # # # # # # # # # # # # #     sm.set_array([])
    # # # # # # # # # # # # # # # #     plt.colorbar(sm, label='k_values')
    # # # # # # # # # # # # # # # #     plt.xlabel('Date')
    # # # # # # # # # # # # # # # #     plt.ylabel('Length Change Rate (m a-1)')
...
def pbs(obs, pred, R):
    ...
    if n_obs == 1:
        ...
        # if isinstance(obs, list) and isinstance(pred, list):
        #     residual = [a - b for a, b in zip(obs, pred)]
        # elif isinstance(obs, np.ndarray) and isinstance(pred, np.ndarray):
        #     residual = obs - pred
        # else:
...
#                     run_opt = False
                    if debug:
                        ...
                        k_value_arrary, reg_calving_gta_mod_array,lengthchange_model_arrary,calving_thickness_model_array,calving_rate_model_array,velocity_at_calvingfront_model_array = Visualize_parameter (model_function = reg_calving_flux, k_bndhigh = calving_k_bndhigh,
                                                                                        ...
                                                                                        frontal_ablation_Gta_cn=frontal_ablation_Gta_cn, 
                                                                                        prms_from_reg_priors=prms_from_reg_priors, prms_from_glac_cal=prms_from_glac_cal,
                                                                                        ignore_nan=False, debug=debug_reg_calving_fxn,calibrate_timeseries=True,sampling_method=1,store_monthly_step=store_monthly_step)
                        #pdb.set_trace() 
                        if store_monthly_step:
                        ...
                        lengthchange_dLdt_unc_obs_ind_2_array = np.array(lengthchnage_dLdt_unc_obs_ind_2).reshape(len(lengthchnage_dLdt_unc_obs_ind_2),1)
                        
                        fa_annual_data_obs_ind_array = np.array(fa_annual_data_obs_ind).reshape(len(fa_annual_data_obs_ind),1)
                        calving_rate_model_array_annual_array = np.array(calving_rate_model_array_annual).reshape(len(calving_rate_model_array_annual), len(calving_rate_model_array_annual[0]))
                        calving_rate_model_array_annual_array = calving_rate_model_array_annual_array.T
                        fa_unc_annual_data_obs_ind_2 = [x**2 for x in fa_unc_annual_data_obs_ind]
                        fa_unc_annual_data_obs_ind_2_array = np.array(fa_unc_annual_data_obs_ind_2 ).reshape(len(fa_unc_annual_data_obs_ind_2 ),1)
                        # TODO set the both fa and dldt as the calibration variables
                        if calibrate_FA and calibrate_dLdt:
                            lengthchange_dLdt_fa_annual_data_obs_ind_array = np.concatenate((lengthchange_dLdt_obs_ind_array,fa_annual_data_obs_ind_array),axis = 0)
                            lengthchange_dLdt_fa_unc_annual_data_obs_ind_2_array = np.concatenate((lengthchange_dLdt_unc_obs_ind_2_array,fa_unc_annual_data_obs_ind_2_array),axis = 0)
                            lengthchange_dLdt_calving_rate_model_array_annual_array = np.concatenate((lengthchange_model_arrary_annual_array,calving_rate_model_array_annual_array),axis = 0)
                            #pdb.set_trace()
                            Weights_k,Neff_k = pbs(lengthchange_dLdt_fa_annual_data_obs_ind_array,lengthchange_dLdt_calving_rate_model_array_annual_array,lengthchange_dLdt_fa_unc_annual_data_obs_ind_2_array)
                        elif calibrate_FA:
                            Weights_k,Neff_k = pbs(fa_annual_data_obs_ind_array,calving_rate_model_array_annual_array,fa_unc_annual_data_obs_ind_2_array)
                        elif calibrate_dLdt:
                            Weights_k, Neff_k = pbs(lengthchange_dLdt_obs_ind_array,lengthchange_model_arrary_annual_array,lengthchange_dLdt_unc_obs_ind_2_array)
                        ...
                        #pdb.set_trace() 
                        Visualization_timeseries.plot_model_vs_observation(length_change_model_we,lengthchange_dLdt_obs_ind,plot_type='timeseries',model_legends= list(map(lambda x: f"{x:.2f}", np.append(k_value_arrary, k_weighted_av))),
                                                                           title ='length change rate comparison model vs observation',xlabel='Year',ylabel = 'length change rate (m a⁻¹)',
                                                                           observation_error=lengthchnage_dLdt_unc_obs_ind,start_date = 2000,save_path=save_path_figure,
                                                                           save_name='length change rate comparison model vs observation (weighted)')
                        #pdb.set_trace()
                        Visualization_timeseries.plot_model_vs_observation(fa_annual_model_we,fa_annual_data_obs_ind,plot_type='timeseries',model_legends= list(map(lambda x: f"{x:.2f}", np.append(k_value_arrary, k_weighted_av))),
                        ...
                        Visualization_timeseries.plot_model_vs_observation((np.append(reg_calving_gta_mod_array_resample,calving_flux_Gta_weighted )).tolist(),reg_calving_gta_obs,plot_type='point',
                                                                            model_label='Modeled frontal ablation (Gt a⁻¹)',obs_label='Observed frontal ablation (Gt a⁻¹)',
                                                                            model_legends=list(map(lambda x: f"{x:.2f}", np.append(k_value_arrary_resample, k_weighted_av))),start_date = 2000,
