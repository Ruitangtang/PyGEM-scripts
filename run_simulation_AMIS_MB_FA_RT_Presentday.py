    ...
    if pygem_prms.sim_iters > 1:
        output_attrs_dict_mad = {
            ...
            'glac_mass_bsl_annual_mad': {
                    ...
                    'temporal_resolution': 'annual',
                    'comment': 'mass of ice below sea level based on area and ice thickness at start of the year'},
            'glac_mass_bwl_annual_mad': {
                    'long_name': 'glacier mass below water level median absolute deviation',
                    'units': 'kg',
                    'temporal_resolution': 'annual',
                    'comment': 'mass of ice below water level based on area and ice thickness at start of the year'},
            'glac_ELA_annual_mad': {
...
def simu_MB_FA(list_packed_vars):
    """
    ...
    for glac in range(main_glac_rgi.shape[0]):
        ...
        # for batman in [0]:
            ...
            if (fls is not None) and (glacier_area_km2.sum() > 0):
                ...
                output_glac_mass_bwl_annual = np.zeros((year_values.shape[0], sim_iters)) * np.nan
                ...
                for n_iter in range(sim_iters):
                    ...
                    if not tau_values is None:
                        tau = tau_values[n_iter]
...
#                            mb_em_sims.append(mb_em_mwea)
            ...
            #                     plt.show()
                            ...
                            if debug:
                                ...
                                print('  vol final [Gt]:', np.round((diag.volume_m3.values[-1]-diag.volume_m3.values[-2]) * 0.9 / 1e9,5))
                        ...
                        if debug:
                            ...
                            print('the glacier wide frontalablation [Gt]is :',mbmod.glac_wide_frontalablation.sum()/1e9)
                        ...
                        try:
                            ...
                            if Dynamic_step_Monthly:
                                ...
                                #pdb.set_trace()
                                output_glac_length_change_annual[:, n_iter] = np.append(length_change_m_annual,diag.length_change_rate_myr.values[-1])
                                output_glac_frontalablation_annual[:, n_iter] = np.append(calving_m3_annual,calving_m3_annual[-1])
                            ...
                            try:
                                if Dynamic_step_Monthly:
                                    ...
                                    volume_bwl_annual = (((diag.volume_bwl_m3.values[:-1]).reshape(-1,12))[:,0]).flatten()
                                    ...
                                    output_glac_mass_bwl_annual[:, n_iter] = (np.append(volume_bwl_annual,diag.volume_bwl_m3.values[-1]))* pygem_prms.density_ice
                                else:
                                    output_glac_area_annual[:, n_iter] = diag.area_m2.values
                                    ...
                                    output_glac_mass_bwl_annual[:, n_iter] = diag.volume_bwl_m3.values * pygem_prms.density_ice
                            except:
                                if debug:
                                    print(traceback.format_exc())
                        ...
                        if output_glac_bin_icethickness_annual is None:
                            try:
                                ...
                                print("------------- record 2 finish ------------- (output_glac_bin_icethickness_annual is None)")
                            except:
                                print(traceback.format_exc())
                        else:
                            print("output_glac_bin_icethickness_annual is Not None:",output_glac_bin_icethickness_annual)
                            # Update the latest thickness and volume
                            output_glac_bin_mass_annual_sim = (mbmod.glac_bin_area_annual *
                                                                 mbmod.glac_bin_icethickness_annual * 
                                                                 pygem_prms.density_ice)[:,:,np.newaxis]
                ...
                if count_exceed_boundary_errors < pygem_prms.sim_iters:
                    ...
                    if pygem_prms.export_essential_data:
                        #if 1:  # TODO Here is just for the test, need to be modified
                        try:
                            #pdb.set_trace()
                            ...
                            output_glac_mass_bwl_annual_stats = calc_stats_array_Restruct(output_glac_mass_bwl_annual,rgiid_ind = rgiid)
                            ...
                            if pygem_prms.export_extra_vars:
                                #TODO check the monthly output, at the moment, if set the dynamic step as monthly/annual, the output is monthly/annual
                            ...
                            #pdb.set_trace()
                            # ==== save the full stats info of the model output
                            output_glac_stats_all_dict = {'glac_runoff_monthly_stats': output_glac_runoff_monthly_stats, 
                                                          'glac_area_annual_stats': output_glac_area_annual_stats,
                                                          'glac_length_annual_stats': output_glac_length_annual_stats,
                                                          'glac_length_change_annual_stats': output_glac_length_change_annual_stats,
                                                          'glac_frontalablation_annual_stats': output_glac_frontalablation_annual_stats,
                                                          'glac_mass_annual_stats': output_glac_mass_annual_stats,
                                                          'glac_mass_bsl_annual_stats': output_glac_mass_bsl_annual_stats,
                                                          'glac_mass_bwl_annual_stats': output_glac_mass_bwl_annual_stats,
                                                          'glac_ELA_annual_stats': output_glac_ELA_annual_stats,
                                                          'offglac_runoff_monthly_stats': output_offglac_runoff_monthly_stats,
                                                          'glac_temp_monthly_stats': output_glac_temp_monthly_stats,
                                                          'glac_prec_monthly_stats': output_glac_prec_monthly_stats,
                                                          'glac_acc_monthly_stats': output_glac_acc_monthly_stats,
                                                          'glac_refreeze_monthly_stats': output_glac_refreeze_monthly_stats,
                                                          'glac_melt_monthly_stats': output_glac_melt_monthly_stats,
                                                          'glac_frontalablation_monthly_stats': output_glac_frontalablation_monthly_stats,
                                                          'glac_length_monthly_stats': output_glac_length_monthly_stats,
                                                          'glac_length_change_monthly_stats': output_glac_length_change_monthly_stats,
                                                          'glac_massbaltotal_monthly_stats': output_glac_massbaltotal_monthly_stats,
                                                          'glac_snowline_monthly_stats': output_glac_snowline_monthly_stats,
                                                          'glac_mass_change_ignored_annual_stats': output_glac_mass_change_ignored_annual_stats,
                                                          'offglac_prec_monthly_stats': output_offglac_prec_monthly_stats,
                                                          'offglac_melt_monthly_stats': output_offglac_melt_monthly_stats,
                                                          'offglac_refreeze_monthly_stats': output_offglac_refreeze_monthly_stats,
                                                          'offglac_snowpack_monthly_stats': output_offglac_snowpack_monthly_stats}
                            # save the full stats info of the model output   
                            # Define the output folder path
                            output_glac_stats_all_folder = os.path.join(
                                pygem_prms.output_sim_fp, 'stats', reg_str, gcm_name, 'AllInfo')
                            # Add scenario subfolder for specific GCMs
                            if gcm_name not in ['ERA-Interim', 'ERA5', 'COAWST']:
                                output_glac_stats_all_folder = os.path.join(output_glac_stats_all_folder, scenario)
                            # Ensure the directory exists
                            os.makedirs(output_glac_stats_all_folder, exist_ok=True)
                            # Define the output JSON filename
                            output_glac_stats_all_fn = f"{glacier_str}_{gcm_name}_{scenario}_{sim_iters}sets_" \
                                                    f"{args.gcm_bc_startyear}_{args.gcm_endyear}_All_stats.json"
                            # Full file path
                            output_glac_stats_all_fp = os.path.join(output_glac_stats_all_folder, output_glac_stats_all_fn)
                            # Save the JSON file with proper serialization
                            with open(output_glac_stats_all_fp, 'w') as f:
                                json.dump(output_glac_stats_all_dict, f, indent=4, default=convert_to_serializable)

                            #pdb.set_trace()
                            # output the selected info mean, mad
                            # Output Mean
                            ...
                            output_ds_all_stats['glac_mass_bwl_annual'].values[0,:] = output_glac_mass_bwl_annual_stats[:,0]
                            ...
                            if pygem_prms.export_extra_vars:
                                ...
                                output_ds_all_stats['glac_length_change_monthly'].values[0,:] = (
                                        output_glac_length_change_monthly_stats[:,0])                                
                            ...
                            if pygem_prms.sim_iters > 1:
                                ...
                                output_ds_all_stats['glac_mass_bwl_annual_mad'].values[0,:] = output_glac_mass_bwl_annual_stats[:,1]
                        ...
                        # ----- EXPORT TO JSON -----
                        output_ds_all_stats_folder = os.path.join(
                            pygem_prms.output_sim_fp, 'stats', reg_str, gcm_name, 'SelecInfo')
                        # Add scenario subfolder for specific GCMs
                        if gcm_name not in ['ERA-Interim', 'ERA5', 'COAWST']:
                            output_ds_all_stats_folder = os.path.join(output_ds_all_stats_folder, scenario)
                        # Ensure the directory exists
                        os.makedirs(output_ds_all_stats_folder, exist_ok=True)
                        # Define output filename
                        output_ds_all_stats_fn = f"{glacier_str}_{gcm_name}_{scenario}_{sim_iters}sets_" \
                                                f"{args.gcm_bc_startyear}_{args.gcm_endyear}_selected_stats.json"
                        # Full file path
                        output_ds_all_stats_fp = os.path.join(output_ds_all_stats_folder, output_ds_all_stats_fn)
                        # Convert xarray dataset to dictionary and save as JSON
                        with open(output_ds_all_stats_fp, 'w') as f:
                            json.dump(output_ds_all_stats.to_dict(), f, indent=4, default=convert_to_serializable)
                        print(f"Saved stats to: {output_ds_all_stats_fp}")

                        # ----- Visulization -----
                                                
                        # visualize the statistics of lengthchange and frontal ablation # TODO set all the visualization in one function in the graphics.py or Visualization_timeseries.py
                        ...
                        #pdb.set_trace()
                        try: #TODO set the start date and end_date as the dynamic variable
                            plot_timeseries_stats(output_glac_length_change_annual,start_date = 2000,end_date = 2032,save_path=save_path_figure,save_name=save_name_lenthchange)
                            plot_timeseries_stats(output_glac_frontalablation_annual,start_date = 2000,end_date = 2032,save_path=save_path_figure,save_name=save_name_frontalablation)

                            save_name_lenthchange_sub = 'lengthchange_sub_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.png'
                            save_name_frontalablation_sub = 'frontalablation_sub_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.png'
                            #pdb.set_trace()
                            plot_timeseries_stats_sub(output_glac_length_change_annual,start_date = 2000,end_date = 2032,save_path=save_path_figure,save_name=save_name_lenthchange_sub)
                            plot_timeseries_stats_sub(output_glac_frontalablation_annual,start_date = 2000,end_date = 2032,save_path=save_path_figure,save_name=save_name_frontalablation_sub)
                        except:
                            print("check the ploting function")
                            print(traceback.format_exc())
                        ...
                        output_glac_length_change_annual_df.to_csv(save_path_modeloutput + 'lengthchange_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.csv')
                        output_glac_frontalablation_annual_df.to_csv(save_path_modeloutput + 'frontalablation_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.csv')
                        #pdb.set_trace()
    ...
    for gcm_name in gcm_list:
        ...
        if args.option_parallels:
            ...
            with multiprocessing.Pool(num_cores) as p:
                p.map(simu_MB_FA,list_packed_vars)
        # If not in parallel, then only should be one loop
        else:
            # Loop through the chunks and export bias adjustments
            for n in range(len(list_packed_vars)):
                simu_MB_FA(list_packed_vars[n])
