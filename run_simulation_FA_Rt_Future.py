            ...
            if (fls is not None) and (glacier_area_km2.sum() > 0):
                ...
                if pygem_prms.use_calibrated_modelparams:
                    ...
                    if not glacier_rgi_table['TermType'] in [1,5] or not pygem_prms.include_calving:
                        ...
                        #TODO read the calving_k_value and the corresponding weights from the run_calibration_FA_Rt_New.py and doing the particle_resampling,here is done by outside and hard read
                ...
                if count_exceed_boundary_errors < pygem_prms.sim_iters:
                    ...
                    if pygem_prms.export_essential_data:
                        try:
                            ...
                            output_glac_length_change_annual_stats = calc_stats_array(output_glac_length_change_annual)
                            output_glac_frontalablation_annual_stats = calc_stats_array(output_glac_frontalablation_annual)
                            ...
                            if pygem_prms.export_extra_vars:
                                ...
                                output_glac_length_change_monthly_stats = calc_stats_array(output_glac_length_change_monthly)
                        ...
                        output_glac_length_change_annual_df.to_csv(save_path_parameter + 'lengthchange_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.csv')
                        output_glac_frontalablation_annual_df.to_csv(save_path_parameter + 'frontalablation_' + glacier_str + '_' + gcm_name + '_' + scenario + '_'  + str(sim_iters) + 'sets' + '_' + str(args.gcm_bc_startyear) + '_' + str(args.gcm_endyear) + '.csv')
                        #pdb.set_trace()
