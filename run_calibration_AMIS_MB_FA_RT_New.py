...
# It's a copy of the run_calibration_AMIS_MB_FA.py, but with a better workflow
...
#%% ----- the path of output -----
output_fp = pygem_prms.main_directory + '/Calibration_AMIS_MB_FA/'
...
def Visualize_parameter_paralle (model_function = None, parameters_dict = None,calibrate_timeseries = False,
                                 rgiid_ind = None, Visual_index = False, Debug_index =False, N_iteration = None,
                                 store_result =False,output_fp =None, **kwargs):
    ...
    # parameters_dict is the dictionary of the parameters, the key is the name of the parameter, the value is the list of the parameter
    ...
    # output_fp, str, the path of the output folder
...
def Model_MB_FA_RT(model_function = reg_calving_flux,parameters_dict= None,rgiid_ind = None,
                   ...
                   debug=True,calibrate_timeseries =True,store_monthly_step=False, return_all = False,
                   store_result= False,output_fp =None,N_iteration = None,**kwargs):
    """
    ...
    return_all : bool, optional, if True, the function will return all the output of the model_function, the default is False, just returned the output for the calibration
    output_fp : str, optional, the path to store the results
    ...
    output_prior =  Visualize_parameter_paralle (model_function = model_function,parameters_dict= parameters_dict,rgiid_ind = rgiid_ind,
                                                                    ...
                                                                    N_iteration=N_iteration,store_result=store_result,output_fp=output_fp,**kwargs)
    # Extract the results
    ...
    if store_result:
        ...
        if store_monthly_step:
            ...
            output_folder_monthly = os.path.join(output_fp, 'modeloutput','Monthly')  # Assuming `pygem_prms.output_fp` exists
        ...
        output_folder_annual = os.path.join(output_fp, 'modeloutput','Annual')  # Assuming `pygem_prms.output_fp` exists
    ...
    # here is the full returen of the model, #TODO add the monthly results, or depends on the user's choice
        return (
            ...
            FA_mwea_TMS_model_array_annual,velocity_at_calvingfront_model_array_annual,thickness_at_calvingfront_model_array_annual,
        ...
        return lengthchange_dLdt_model_array_annual, calving_flux_Gta_TMS_model_array_annual,calving_flux_Gta_average_model_array,massbalclim_model_array,mb_obs_mwea,mb_obs_mwea_err
...
#                output_df_all.loc[nglac, 'thick_measured_yn'] = fa_glac_data_reg.loc[fa_idx, 'thick_measured_yn']
            ...
            for nglac in np.arange(main_glac_rgi.shape[0]):
                ...
                for j in range(max_iterations): #TODO DO THE APATED CALIBRATION HERE
                    ...
                    lengthchange_dLdt_model_array_annual, calving_flux_Gta_TMS_model_array_annual,calving_flux_Gta_average_model_array,massbalclim_model_array,mb_obs_mwea,mb_obs_mwea_err = Model_MB_FA_RT(model_function = reg_calving_flux,parameters_dict= parameters_dict,
                                                                                         rgiid_ind = rgiid_ind,main_glac_rgi = main_glac_rgi_ind, fa_glac_data_reg= fa_glac_data_ind,
                                                                                         ignore_nan=False,debug=debug,calibrate_timeseries =True,store_monthly_step=store_monthly_step,
                                                                                         return_all = False,store_result= True,output_fp = output_fp,N_iteration = j)
                    ...
                    predicted = lengthchange_dLdt_MB_model_array_annual_array_1_10
                    observations_sbst_masked = lengthchange_dLdt_annual_CMB_obs_ind_array_1_10
                    r_cov = lengthchange_dLdt_annual_CMB_obs_ind_2_array_1_10
                    ...
                    output_folder_AMIS = os.path.join(output_fp,'AMIS_info') # Assuming `pygem_prms.output_fp` exists
                ...
                if (not doadapt) or (not notlast): # TODO add the information of the txt infor about good and maximum iterations, with the Neff
                    ...
                    output_folder_params_poster = os.path.join(output_fp, 'parameter','Poster')
                    # Ensure directories exist
                    ...
                    output_folder_post_params_statis = os.path.join(output_fp, 'parameter')  # Assuming `pygem_prms.output_fp` exists
                    # Check if directory exists, otherwise create it
                    ...
                    calving_flux_Gta_TMS_model_array_annual_post,FA_mwea_TMS_model_array_annual_post, 
                    velocity_at_calvingfront_model_array_annual_post, thickness_at_calvingfront_model_array_annual_post,
                    ...
                    mb_obs_mwea, mb_obs_mwea_err
                                       ...
                                       parameters_dict=parameters_dict_post,
                                       rgiid_ind=rgiid_ind,
                    ...
                    #pdb.set_trace()
                    calving_flux_Gta_average_model_weighted = np.average(calving_flux_Gta_average_model_array_post)
                    calving_thickness_model_weighted = np.average(calving_thickness_model_array_post)
                    massbalclim_model_weighted = np.average(massbalclim_model_array_post)
                    massbaltotal_model_weighted = np.average(massbaltotal_model_array_post)
                    FA_mwea_average_model_weighted = np.average(FA_mwea_average_model_array_post)

                    lengthchange_dLdt_model_annual_weighted = np.average(lengthchange_dLdt_model_array_annual_post,axis = 1)
                    lengthchange_m_TMS_model_annual_weighted = np.average(lengthchange_m_TMS_model_array_annual_post,axis = 1)
                    calving_flux_Gta_TMS_model_annual_weighted = np.average(calving_flux_Gta_TMS_model_array_annual_post,axis = 1)
                    # massbalclim_TMS_model_annual_weighted = np.average(massbalclim_TMS_model_array_annual_post,axis =1)
                    # massbaltotal_TMS_model_annual_weighted = np.average(massbaltotal_TMS_model_array_annual_post,axis =1)
                    FA_mwea_TMS_model_annual_weighted = np.average(FA_mwea_TMS_model_array_annual_post,axis =1)
                    velocity_at_calvingfront_model_array_annual_weighted = np.average(velocity_at_calvingfront_model_array_annual_post,axis =1)
                    thickness_at_calvingfront_model_array_annual_weighted = np.average(thickness_at_calvingfront_model_array_annual_post,axis =1)
                    width_at_calvingfront_model_array_annual_weighted = np.average(width_at_calvingfront_model_array_annual_post,axis =1)
                    volume_bsl_model_array_annual_weighted = np.average(volume_bsl_model_array_annual_post,axis =1)
                    volume_bwl_model_array_annual_weighted = np.average(volume_bwl_model_array_annual_post,axis =1)

                    # ==== Save the weighted results in json
                    output_folder_weights = os.path.join(output_fp, 'modeloutput','Poster', 'Weighted')  # Assuming `pygem_prms.output_fp` exists
                    # Check if directory exists, otherwise create it
                    ...
                    try:
                        if Visualize_Index:
                            ...
                            Visualization_timeseries.plot_model_vs_observation((np.append(calving_flux_Gta_average_model_array_post, calving_flux_Gta_average_model_weighted)).tolist(),
                                                                            fa_gta_obs_ind_array, plot_type='point', model_label='Modeled frontal ablation (Gt a⁻¹)', obs_label='Observed frontal ablation (Gt a⁻¹)',
                                                                                model_legends=[f"Particle {i+1}" for i in range(30)] + ["Weighted Avg"], start_date=2000,
                            ...
                            Visualization_timeseries.plot_model_vs_observation([*lengthchange_dLdt_model_array_annual_post.T, lengthchange_dLdt_model_annual_weighted], 
                                                                            lengthchange_dLdt_obs_ind, plot_type='timeseries',
                                                                            model_legends=[f"Particle {i+1}" for i in range(30)] + ["Weighted Avg"],
                                                                            ...
                                                                            start_date=2000, save_path=save_path_figure, save_name='length_change_rate_comparison')
                            # length change m (the difference of the length of the elevation-band flowlines)vs observation
                            Visualization_timeseries.plot_model_vs_observation([*lengthchange_m_TMS_model_array_annual_post.T,
                                                                                lengthchange_m_TMS_model_annual_weighted], lengthchange_dLdt_obs_ind,
                                                                                plot_type='timeseries', model_legends=[f"Particle {i+1}" for i in range(30)] + ["Weighted Avg"],
                            ...
                            Visualization_timeseries.plot_model_vs_observation((np.append(massbalclim_model_array_post, massbalclim_model_weighted)).tolist(),
                                                                            mb_obs_mwea, plot_type='point', model_legends=[f"Particle {i+1}" for i in range(30)] + ["Weighted Avg"],
                    ...
                    if store_monthly_step:
                        ...
                        #pdb.set_trace()
                        lengthchange_dLdt_model_array_monthly_post_weighted = np.average(lengthchange_dLdt_model_array_monthly_post, axis=0)
                        lengthchange_m_TMS_model_array_monthly_post_weighted = np.average(lengthchange_m_TMS_model_array_monthly_post, axis=0)
                        calving_flux_Gta_TMS_model_array_monthly_post_weighted = np.average(calving_flux_Gta_TMS_model_array_monthly_post, axis=0)
                        FA_mwea_TMS_model_array_monthly_post_weighted = np.average(FA_mwea_TMS_model_array_monthly_post, axis=0)
                        velocity_at_calvingfront_model_array_monthly_post_weighted = np.average(velocity_at_calvingfront_model_array_monthly_post, axis=0)
                        thickness_at_calvingfront_model_array_monthly_post_weighted = np.average(thickness_at_calvingfront_model_array_monthly_post, axis=0)
                        width_at_calvingfront_model_array_monthly_post_weighted = np.average(width_at_calvingfront_model_array_monthly_post, axis=0)
                        volume_bsl_model_array_monthly_post_weighted = np.average(volume_bsl_model_array_monthly_post, axis=0)
                        volume_bwl_model_array_monthly_post_weighted = np.average(volume_bwl_model_array_monthly_post, axis=0)
                        #massbalclim_TMS_model_array_monthly_post_weighted = np.average(np.asarray(massbalclim_TMS_model_array_monthly), axis=0)
                        #massbaltotal_TMS_model_array_monthly_post_weighted = np.average(np.asarray(massbaltotal_TMS_model_array_monthly), axis=0)

                        # --- save the monthly information in a hdf5 file and visulize the monthly information
                        output_folder_monthly_weighted = os.path.join(output_fp, 'modeloutput', 'Poster','Weighted','Monthly')
            ...
            if len(Failed_glacs) > 0:
                failed_glaciers_fp = os.path.join(output_fp, 'Failed_glaciers.txt')
            ...
            if Good_AMIS:
                good_amis_fp = os.path.join(output_fp, 'Good_AMIS.txt')
                with open(good_amis_fp, 'w') as f:
                    f.write(f'There are {N_good_AMIS} glaciers that passed calibration\n')
                    f.writelines(f"{glacier}\n" for glacier in Good_AMIS)

            
            if Bad_AMIS:
                bad_amis_fp = os.path.join(output_fp, 'Bad_AMIS.txt')
...
def main():
    ...
    output_fp = os.path.join(base_dir,'Calibration_AMIS_MB_FA')
