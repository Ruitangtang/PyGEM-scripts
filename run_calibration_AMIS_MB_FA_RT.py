...
def Visualize_parameter_paralle (model_function = None, parameters_dict = None,calibrate_timeseries = False,rgiid_ind = None, Visual_index = False, Debug_index =False, **kwargs):
...
#                output_df_all.loc[nglac, 'thick_measured_yn'] = fa_glac_data_reg.loc[fa_idx, 'thick_measured_yn']
            ...
            for nglac in np.arange(main_glac_rgi.shape[0]):
                ...
                for j in range(max_iterations): #TODO DO THE APATED CALIBRATION HERE
                    ...
                    # Generate the particles
                    param_prior_array,output_prior =  Visualize_parameter_paralle (model_function = reg_calving_flux,parameters_dict= parameters_dict,rgiid_ind = rgiid_ind,
                    ...
                    if j == 0:
                        parameters_dict_noIndex = {key: value for key, value in param_prior_array.items() if key != "index"} 
                    ...
                    print('Neff: {Neff_k} in j:{j}'.format(Neff_k=int(Neff_k),
                                                        j=j))
                ...
                #%% rerun the model with the param_post_array
                ...
                dataset_dict_weights = {'param_prior_array_Tbias_kp_ddfsnow_tau': param_prior_array,
                                        'param_post_array_Tbias_kp_ddfsnow_tau': param_post_array,
                ...
                if Neff_k > 1:
                    ...
                    try:
                        param_weighted_av = np.average(param_post_array,weights = Weights_k) #TODO check the shape of the param_post_array and Weights_k
                        param_weighted_std = np.sqrt(np.average((param_post_array - param_weighted_av)**2,weights = Weights_k))
                        # period average
                        ...
                        if Visualize_Index:
                            ...
                            Visualization_timeseries.plot_model_vs_observation((np.append(calving_flux_Gta_average_model_array,calving_flux_Gta_average_model_weighted )).tolist(),fa_gta_obs_ind_array,plot_type='point',
                                                    ...
                                                    model_legends= list(map(lambda x: f"{x:.2f}", np.append(param_post_array, param_weighted_av))),start_date = 2000,
                            ...
                            Visualization_timeseries.plot_model_vs_observation([lengthchange_dLdt_model_array_annual,lengthchange_dLdt_model_annual_weighted],
                                                                            ...
                                                                            model_legends=list(map(lambda x: f"{x:.2f}", np.append(param_post_array, param_weighted_av))),
                            ...
                            param_post_array_resample_index = np.random.choice(len(param_post_array), size = size_resample, p = Weights_k)
                            param_post_array_resample = param_post_array[param_post_array_resample_index]
                            calving_flux_Gta_average_model_array_resample = calving_flux_Gta_average_model_array[param_post_array_resample_index]
                            massbalclim_model_array_resample = massbalclim_model_array[param_post_array_resample_index]
                            massbaltotal_model_array_resample = massbaltotal_model_array.T[param_post_array_resample_index]
                            FA_mwea_average_model_array_resample = FA_mwea_average_model_array.T[param_post_array_resample_index]
                            calving_thickness_model_array_resample = calving_thickness_model_array[param_post_array_resample_index]

                            lengthchange_dLdt_model_array_annual_resample = ((lengthchange_dLdt_model_array_annual.T)[param_post_array_resample_index]).T
                            lengthchange_m_TMS_model_array_annual_resample = ((lengthchange_m_TMS_model_array_annual.T)[param_post_array_resample_index]).T
                            calving_flux_Gta_TMS_model_array_annual_resample = ((calving_flux_Gta_TMS_model_array_annual.T)[param_post_array_resample_index]).T                            
                            FA_mwea_TMS_model_array_resample = ((FA_mwea_TMS_model_array.T)[param_post_array_resample_index]).T
                            velocity_at_calvingfront_model_array_annual_resample = ((velocity_at_calvingfront_model_array_annual.T)[param_post_array_resample_index]).T
                            thickness_at_calvingfront_model_array_annual_resample = ((thickness_at_calvingfront_model_array.T)[param_post_array_resample_index]).T
                            width_at_calvingfront_model_array_annual_resample = ((width_at_calvingfront_model_array.T)[param_post_array_resample_index]).T
                            volume_bsl_model_array_annual_resample = ((volume_bsl_model_array_annual.T)[param_post_array_resample_index]).T
                            volume_bwl_model_array_annual_resample = ((volume_bwl_model_array_annual.T)[param_post_array_resample_index]).T
                            # massbalclim_TMS_model_array_annual_resample = ((massbalclim_TMS_model_array_annual.T)[param_post_array_resample_index]).T
                            # massbaltotal_TMS_model_array_annual_resample = ((massbaltotal_TMS_model_array_annual.T)[param_post_array_resample_index]).T
                            ...
                            output_data_dict_resample = {
                                'param_post_array_resample': param_post_array_resample,
                            ...
                            if Visualize_Index:
                                ...
                                Visualization_timeseries.plot_model_vs_observation((np.append(calving_flux_Gta_average_model_array_resample,calving_flux_Gta_average_model_weighted )).tolist(),fa_gta_obs_ind_array,plot_type='point',
                                                        model_label='Modeled frontal ablation (Gt a⁻¹)',obs_label='Observed frontal ablation (Gt a⁻¹)',
                                                        model_legends= list(map(lambda x: f"{x:.2f}", np.append(param_post_array_resample, param_weighted_av))),start_date = 2000,
                                                        title='Calving flux comparison model vs observation',observation_error=fa_gta_obs_unc_ind,
                                ...
                                Visualization_timeseries.plot_model_vs_observation([lengthchange_dLdt_model_array_annual_resample,lengthchange_dLdt_model_annual_weighted],
                                                                                lengthchange_dLdt_obs_ind,plot_type='timeseries',
                                                                                model_legends=list(map(lambda x: f"{x:.2f}", np.append(param_post_array_resample, param_weighted_av))),
                                                                                title ='length change rate (dLdt)comparison model vs observation',xlabel='Year',
                                                                                ylabel = 'length change rate (m a⁻¹)',observation_error=lengthchnage_dLdt_unc_obs_ind,start_date = 2000,
                            ...
                            if store_monthly_step:
                                ...
                                output_data_dict_monthly = {
                                    ...
                                    'param_post_array': param_post_array,
                                    'calving_flux_Gta_average_model_array': calving_flux_Gta_average_model_array,
                                ...
                                # === Resampled
                                lengthchange_dLdt_model_array_monthly_resample = lengthchange_dLdt_model_array_monthly[:,param_post_array_resample_index]
                                lengthchange_m_TMS_model_array_monthly_resample = lengthchange_m_TMS_model_array_monthly[:,param_post_array_resample_index]
                                calving_flux_Gta_TMS_model_array_monthly_resample = calving_flux_Gta_TMS_model_array_monthly[:,param_post_array_resample_index]
                                FA_mwea_TMS_model_array_monthly_resample = FA_mwea_TMS_model_array_monthly[:,param_post_array_resample_index]
                                velocity_at_calvingfront_model_array_monthly_resample = velocity_at_calvingfront_model_array_monthly[:,param_post_array_resample_index]
                                thickness_at_calvingfront_model_array_monthly_resample = thickness_at_calvingfront_model_array_monthly[:,param_post_array_resample_index]
                                width_at_calvingfront_model_array_monthly_resample = width_at_calvingfront_model_array_monthly[:,param_post_array_resample_index]
                                volume_bsl_model_array_monthly_resample = volume_bsl_model_array_monthly[:,param_post_array_resample_index]
                                volume_bwl_model_array_monthly_resample = volume_bwl_model_array_monthly[:,param_post_array_resample_index]
                                #massbalclim_TMS_model_array_monthly_resample = massbalclim_TMS_model_array_monthly[:,param_post_array_resample_index]
                                #massbaltotal_TMS_model_array_monthly_resample = massbaltotal_TMS_model_array_monthly[:,param_post_array_resample_index]
                                # --- save the monthly information in a hdf5 file and visulize the monthly information
